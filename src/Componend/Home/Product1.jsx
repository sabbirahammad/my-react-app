...
const ProductShowcase = () => {
  ...
  .filter((product) => {
    ...
    if (matchedCategory?.name?.toLowerCase() === 'men') {
      ...
      <div className="max-w-screen-xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-9 gap-4 sm:gap-6">
          ...
          <div className="col-span-1 sm:col-span-2 lg:col-span-3 grid grid-cols-2 gap-4">
            {categoryProducts.slice(1, 4).map((product, index) => (
              <SmallProductCard
                key={product.id}
                product={product}
                index={index}
          ...
          {categoryProducts[0] && (
            <motion.div
              ...
              onClick={() => handleCardClick(categoryProducts[0])}
            >
              <img
                src={categoryProducts[0].images?.[0] || 'https://via.placeholder.com/300x200?text=No+Image'}
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
                ...
                <p className="text-yellow-400 text-sm sm:text-base font-medium">
                  ৳{categoryProducts[0].price}
                </p>
                <p className="text-gray-400 text-xs sm:text-sm line-through">
                  ৳{categoryProducts[0].oldPrice}
                </p>
                <motion.button
                  onClick={(e) => handleViewClick(e, categoryProducts[0])}
          ...
          {/* Right Small Cards */}
          <div className="col-span-1 sm:col-span-2 lg:col-span-3 grid grid-cols-2 gap-4">
            {categoryProducts.slice(4, 8).map((product, index) => (
              <SmallProductCard
                key={product.id}
                product={product}
                index={index}
...
const SmallProductCard = ({ product, index, handleCardClick, handleViewClick }) => {
